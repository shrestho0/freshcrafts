...
export const PATCH: RequestHandler = async ({ request, fetch }) => {
    ...
    try {
        ...
        console.warn("[DEBUG]: PATCH Request on Notification SSE\nData: ", JSON.stringify(data, null, 2), "\n");

        // check if all keys are present
        if (!data.id || !data.message || !data.actionHints || !data.type) throw new Error("Invalid payload")
        ...
        console.warn(`[DEBUG]: Requested ${locationHint}, and received: `, JSON.stringify(res, null, 2), "\n");
