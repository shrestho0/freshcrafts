...
import { InlineLoading, Search, SelectableTile, Tile } from 'carbon-components-svelte';
import { ArrowRightIcon } from 'lucide-svelte';
import { onMount } from 'svelte';
import { Query, Locked, Unlocked } from 'carbon-icons-svelte';
	...
	if (search_query.trim() === '') {
		...
		if (original_repo_data?.repos?.length > 0) {
			for (const repo of original_repo_data.repos) {
				if (repo?.name?.toLowerCase().includes(search_query?.toLowerCase())) {
...
async function handleContinue() {
	...
	if (savedGithubStuff.selected_repo) {
		window.addEventListener('beforeunload', function (e) {
			if (savedGithubStuff.select_status == 'leaving_page') return null;
			e.preventDefault();
		});
		...
		const res = await fetch('', {
			...
			headers: {
				...
				'x-freshcraft-project-type': InternalNewProjectType.CREATE_PROJECT_FROM_GITHUB
			},
			body: JSON.stringify({
			...
			{#each repos_to_show as repo, idx}
				...
				>
					<div class=" flex flex-col flex-col">
						...
						<div class="font-light">{humanizedTimeDifference(new Date(repo['pushed_at']))}</div>
					</div>
				</SelectableTile>
			...
			>
				<div class=" flex flex-col flex-col">
					...
					<div class="font-light">
						{humanizedTimeDifference(new Date(savedGithubStuff.selected_repo['pushed_at']))}
					</div>
				</div>
		...
		{:else if savedGithubStuff.select_status == 'repo_uploading'}
			...
			<Tile class="w-full flex flex-col items-center justify-center">
				...
				<div class="text-center text-[var(--cds-interactive-02)]">
					<div class="text-lg font-normal">Downloading repository from github.</div>
