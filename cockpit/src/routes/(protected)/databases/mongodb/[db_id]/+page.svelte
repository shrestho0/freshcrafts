...
import type { EngineCommonResponseDto, EngineMongoDBGetOneError } from '@/types/dtos';
	...
	{#if data?.success}
				...
				<Tile class=" ">
					...
					<div class="the-conn-string w-full">
						...
						{:else if connectionOptions.selectedIdx == 2}
							<CodeSnippet type="multi" light={true}>
								{generateMongoDBEnv(data.payload, connectionOptions.remote_or_local === 'remote')}
							</CodeSnippet>
						{:else if connectionOptions.selectedIdx == 3}
							<div class="flex flex-col gap-2 py-2">
								Connection Host:
								<CodeSnippet light={true} type="single">
									{getMongoDBConnectionHost(connectionOptions.remote_or_local === 'remote')}
								</CodeSnippet>
								Connection Port:
								<CodeSnippet light={true} type="single">
									{getMongoDBConnectionPort(connectionOptions.remote_or_local === 'remote')}
								</CodeSnippet>
								DB Name:
								<CodeSnippet light={true} type="single">
									{data.payload.dbName}
		...
		{:else if data.payload.status === DBMongoStatus.FAILED}
			<Tile class="w-full flex flex-col items-center justify-center">
				...
				<div class="text-center text-[var(--cds-interactive-02)]">
					<div class="text-lg font-normal">Failed to create MongoDB Database</div>
		...
		{:else if data.payload.status === DBMongoStatus.UPDATE_FAILED}
			<Tile class="w-full flex flex-col items-center justify-center">
				<OctagonX class=" h-12 h-12" />
				<div class="text-center text-[var(--cds-interactive-02)]">
					<div class="text-lg font-normal">Failed to update MongoDB Database</div>
					<div class="text-md py-1">
