...
import type { EngineCommonResponseDto, EnginePostgreSQLGetOneError } from '@/types/dtos';
...
import {
	generatePostgreSQLShellCode,
	generatePostgreSQLConnUrl,
	generatePostgreSQLEnv,
	getPostgreSQLConnectionHost,
	getPostgreSQLConnectionPort
} from '@/utils/db-stuff';

export let data: EngineCommonResponseDto<DBPostgres, EnginePostgreSQLGetOneError>;
...
if (browser && data?.payload?.status != DBPostgresStatus.OK) {
	...
	const sse_url = `/sse/databases/postgres/${db_id}`;
...
<div class="w-full">
	{#if data?.success}
		{#if data.payload.status === DBPostgresStatus.OK}
			<div class="w-full">
				<Tile class=" ">
						...
						{:else if connectionOptions.selectedIdx == 3}
							<div class="flex flex-col gap-2 py-2">
								...
								<CodeSnippet light={true} type="single">
									{getPostgreSQLConnectionHost(connectionOptions.remote_or_local === 'remote')}
								</CodeSnippet>
								Connection Port:
								<CodeSnippet light={true} type="single">
									{getPostgreSQLConnectionPort(connectionOptions.remote_or_local === 'remote')}
								</CodeSnippet>
								DB Name:
								<CodeSnippet light={true} type="single">
									{data.payload.dbName}
			...
			>
				PostgreSQL Shell Access (Coming Soon)
			</div>
		{:else if data.payload.status === DBPostgresStatus.FAILED}
			<Tile class="w-full flex flex-col items-center justify-center">
				...
				<div class="text-center text-[var(--cds-interactive-02)]">
					<div class="text-lg font-normal">Failed to create PostgreSQL Database</div>
		...
		{:else if data.payload.status === DBPostgresStatus.REQUESTED}
			<Tile class="w-full flex flex-col items-center justify-center">
		...
		{:else if data.payload.status === DBPostgresStatus.UPDATE_FAILED}
			<Tile class="w-full flex flex-col items-center justify-center">
				<OctagonX class=" h-12 h-12" />
				<div class="text-center text-[var(--cds-interactive-02)]">
					<div class="text-lg font-normal">Failed to update PostgreSQL Database</div>
					<div class="text-md py-1">
