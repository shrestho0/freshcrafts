<script>
    ...
    const depServiceStatus =
        data?.dependenciesStatus?.secondary_postgres == "true";
    const systemServiceStatus =
        data?.systemServiceStatus?.wizard_postgres == "true";

    const depServiceName = "Secondary_Postgres";
    const systemServiceName = "Wizard_Postgres";
</script>
