<script lang="ts">
	...
	import type { EngineRedisUpdateError } from "@/types/dtos";
	...
	let errors: EngineRedisUpdateError & {
		reset: () => void;
	} = {
		// newDBName: "",
		// newDBUser: "",
			...
			switch (result.type) {
				...
				case "failure":
					...
					if (result?.data?.errors) {
						...
						// 	errors.newUserPassword =
						// 		result?.data?.errors?.newUserPassword;
						// }

						if (result?.data?.errors?.password) {
							errors.password = result?.data?.errors?.password;
						}
					}
					break;
			}
