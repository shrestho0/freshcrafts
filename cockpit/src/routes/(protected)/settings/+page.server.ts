...
import { AUTH_COOKIE_EXPIRES_IN, AUTH_COOKIE_NAME, OAUTH_STATE_COOKIE_NAME } from "$env/static/private";
    ...
    changeInfo: async ({ request, cookies, locals }) => {
        ...
        if (res.success) {
            ...
            if (t.success == true) {
                ...
                cookies.set(AUTH_COOKIE_NAME, JSON.stringify(t.tokens), { path: '/', maxAge: parseInt(AUTH_COOKIE_EXPIRES_IN) });
            }
        }
