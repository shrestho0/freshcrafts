<script lang="ts">
	...
	async function fetchData(p = 1, pageSize = 10) {
		...
		const d = (await fetch(url.toString(), {
			...
			res.json(),
		)) as EnginePaginatedDto<SystemwideNotification>;
...
{#if pageData.content?.length > 0}
	...
	<!-- <div
		...
		> -->
	{#each pageData.content as notification, idx}
		<div
			class="flex items-center justify-between rounded my-2 select-none bx--tile"
		>
			<div class="flex items-center gap-2">
				<!-- icon={!chat.markedAsRead ? CircleSolid : undefined} -->
				<Tag type={generateTagColor(notification.type)}
					>{toTitleCase(notification.type)}</Tag
				>
				{notification.message}
			</div>
			<div class="flex gap-3">
				{humanizedTimeDifference(ulidToDate(notification.id))}
				<a href={actionHintToUrl(notification.actionHints)}>
