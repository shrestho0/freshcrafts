import { AUTH_COOKIE_EXPIRES_IN, AUTH_COOKIE_NAME, GITHUB_CLIENT_ID, GITHUB_CLIENT_SECRET, OAUTH_STATE_COOKIE_NAME } from "$env/static/private";
...
export const load: PageServerLoad = async ({ locals, url, cookies, params, }) => {
    ...
    }).then(res => res.json()).catch(err => { console.log("gh_user_emails_err", err); return null })
