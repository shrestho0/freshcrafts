...
export let dep: ProjectDeployment | null;
...
async function listSourceFile() {
	...
	const res = await fetch('/projects', {
		...
		headers: {
			...
			'x-freshcrafts-project-setup-command': ProjectSetupCommand.LIST_SOURCE_FILES
		},
	...
	if (res?.success) {
		...
		projectRootSelectionErrorMessage = res?.message || 'Error fetching source files';
		projectRootSelectionStatus = 'error';
	...
	<ExpandableSection bind:open={expanded} title="Select Project Root">
		<Tile>
			...
			{:else if projectRootSelectionStatus == 'success'}
				<FileTree bind:items={projectSourceList} bind:selectedFileRelativeUrl />
			{:else}
