<script lang="ts">
	...
	import FromGithub from "@/components/project-specifics/files/FromGithub.svelte";
		...
		{#each items as item, idx}
			<SelectableTile
				...
				selected={selected != null &&
					items[selected].value === item.value}
	...
	{:else if items[selected].value === "fromGithub"}
		...
		{:else}
			<Tile class="flex items-center justify-center text-lg gap-2">
				...
				<a
					href="/settings"
					class="text-[var(--cds-link-01)] hover:text-[var(--cds-link-02)]"
					>settings</a
				> page
			</Tile>
		{/if}
